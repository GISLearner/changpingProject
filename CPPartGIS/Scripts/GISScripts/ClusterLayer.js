  ...
  PopupTemplate, GraphicsLayer
    ...
    return declare([GraphicsLayer], {
        constructor: function (options) {
            ...
            this._singleSym = options.singleSymbol || new SMS("circle", 6, null, new Color("#0FF"));
...
//                alert("Sorry, that cluster contains more than " + this._maxSingles + " points. Zoom in for more detail.");
            	  alert("聚合点超过了"+this._maxSingles+"个，请放大级别或者调整最大值");
                return;
            } else {
                // stop the click from bubbling to the map
                e.stopPropagation();
				// this._map.setLevel(this._map.getLevel() + 1);
...
//                 this._addSingles(singles);
        ...
        _clusterGraphics: function () {
            ...
            for (var j = 0, jl = this._clusterData.length; j < jl; j++) {
                ...
                if(point.x > map.extent.xmin && point.x < map.extent.xmax && point.y > map.extent.ymin && point.y < map.extent.ymax){
                	for (var i = 0; i < this._clusters.length; i++) {
                        var c = this._clusters[i];
        ...
        _addSingles: function (singles) {
            ...
            arrayUtils.forEach(singles, function (p) {
            	if(p.x > map.extent.xmin && p.x < map.extent.xmax && p.y > map.extent.ymin && p.y < map.extent.ymax){
            		var g = new Graphic(
            ...
            var that = this;
            this._map.infoWindow.on("hide", function () {
                for (var g = 0; g < that._singles.length; g++) {
                    var gra = that._singles[g];
                    that.remove(gra);
                }
            });
        },
